...
非技术用户使用 UI 0.8.4，请先阅读仓库根目录 [START_HERE.md](../../../START_HERE.md)，按按钮完成个人账号登录、创建和验收。下文为维护者配置入口说明；真实 Windows/GitHub/资产云验收状态见 [上线与联调说明](上线与联调说明.md)。
...
当前执行器为 0.4.5、界面为 0.8.4。自动化覆盖实际本地 Git、连续领域、故障注入与恢复；资产云 CLI 契约读取证据单独保留。已有 Windows/GitHub 实际创建记录，本版对 sample4 暴露的重复下载问题完成通用修复，但仍需在用户电脑上完成恢复复验。测试环境说明只描述证据来源，不要求用户使用 Linux。
