...
非技术用户使用 UI 0.9.0，请先阅读仓库根目录 [START_HERE.md](../../../START_HERE.md)，按按钮完成个人账号登录、创建和验收。下文为维护者配置入口说明；真实 Windows/GitHub/资产云验收状态见 [上线与联调说明](上线与联调说明.md)。

## 推荐从中文界面开始

中文创建向导已覆盖日常新建。日常用户不需要复制本页命令：按照 [从这里开始](../../../START_HERE.md) 完成首次配置，之后双击“启动向导.bat”，在页面填写需求、审阅、执行并记录验收意见。下文命令保留给维护者和其他维护场景。

界面支持本地新建，也支持本人账号或本人管理的测试组织中的真实公开 GitHub 创建。真实 GitHub 结果必须以页面的远端提交核验为准；某次 Windows 网络失败不应写成通过。

## 你会得到什么

这个插件把量潮已有的对话流程变成可重复运行的配置：先整理你的需求，展示要建立或修改的仓库，确认后执行，最后检查结果。

当前执行器为 0.5.0、界面为 0.9.0。自动化覆盖实际本地 Git、连续领域、故障注入与恢复；资产云 CLI 契约读取证据单独保留。本版补充关键记录写入闸门、跨进程互斥、旧 0.8.6 计划兼容、结构化恢复与成果总览；仍需在用户电脑上完成 Windows 文件占用和真实 GitHub 恢复复验。测试环境说明只描述证据来源，不要求用户使用 Linux。
