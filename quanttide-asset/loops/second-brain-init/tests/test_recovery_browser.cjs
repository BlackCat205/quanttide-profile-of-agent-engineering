...
(async()=>{
 ...
 await page.evaluate(()=>{view={status:'paused',total:0,completed:0,events:[],can_resume:false,error:'暂时无法读取章程'};render()});
 ...
 await page.evaluate(()=>{view={status:'paused',provider:'github',total:27,completed:27,events:[],can_resume:false,error:'最终检查尚未生成'};render()});
 ...
 await page.evaluate(()=>{view={status:'paused',legacy_partial:true,total:27,completed:4,events:[],can_resume:false,error:'旧任务待核对',partial_proposal:{id:'fixture',organization:'Example',repo:'test',repository_id:123,commit:'abc',checked_at:'2026-09-09',content:'# test'}};render()});
