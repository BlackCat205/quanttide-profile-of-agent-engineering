...
(async()=>{
 ...
 await page.screenshot({path:path.join(out,'00-invalid-name.png'),fullPage:true});
...
await page.screenshot({path:path.join(out,'01-create.png'),fullPage:true});
 check(await page.locator('#test-banner').isVisible(),'UI identifies automated feedback as simulated');
 await page.setViewportSize({width:390,height:844});check(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'Mobile form fits viewport');await page.screenshot({path:path.join(out,'02-mobile.png'),fullPage:true});await page.setViewportSize({width:1440,height:1000});
 ...
 check((await page.locator('#document-content').textContent()).includes('相邻领域分工'),'Actual domain README opens in readable dialog');await page.screenshot({path:path.join(out,'04-home.png'),fullPage:true});await page.locator('#close-document').click();
 ...
 await page.screenshot({path:path.join(out,'05-acceptance.png'),fullPage:true});
 ...
 await page.screenshot({path:path.join(out,'06-repeat-create-mobile.png'),fullPage:true});
 ...
 await page.setViewportSize({width:1440,height:1000});await page.screenshot({path:path.join(out,'07-return-to-form.png'),fullPage:true});
 check(errors.length===0,'No JavaScript runtime errors');
 const key=firstKey;const run=path.join(storage,'runs',key);
 for(const file of ['request.yaml','ui.json','ui-report.json','human-acceptance.json','approval-record.json','execution-log.json','verification-report.json'])fs.copyFileSync(path.join(run,file),path.join(out,file));
