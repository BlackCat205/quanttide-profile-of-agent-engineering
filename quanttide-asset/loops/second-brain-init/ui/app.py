...
class Studio:
    def __init__(self,storage,enable_github=False,test_mode=False):
        ...
        self.guard=threading.RLock();self.active=set()
    ...
    def create(self,payload):
        ...
        # Each UI creation is isolated. Maintenance of an old workspace stays in the CLI.
        ...
        with self.guard:
            ...
            if not resume:
                ...
                if plan['provider']=='github':
                    e.require(payload.get('github_confirmation')==plan['organization'],'请输入目标组织名以确认公开 GitHub 写入范围。')
    ...
    def build_report(self,folder):
        ...
        item('mounts','仓库连接是否正确','6 个配套仓库连接到领域，领域连接到总入口；地址和版本指针均正确。',f'{sum(bool(x["passed"]) for x in mounts)}/{len(mounts)} 项连接检查通过。',len(mounts)==7 and all(x['passed'] for x in mounts),B,[f'{target}/.gitmodules','quanttide/.gitmodules'])
        root=provider.repo('quanttide');root_text=e.text_at(root,'README.md');index=e.text_at(root,'domains/README.md')
        ...
        item('root','从总入口能否找到新领域','总首页与领域索引列出本领域，根仓库存在领域连接。','已找到 '+target if registered and idx_ok else '总入口登记或索引缺失。',registered and idx_ok,B,['quanttide/README.md','quanttide/domains/README.md'])
...
def main():
    ...
    p.add_argument('--enable-github',action='store_true',help='维护者启用真实 GitHub 表单；需已登录 gh 与 Git')
    ...
    server=ThreadingHTTPServer(('127.0.0.1',args.port),Handler);server.token=secrets.token_urlsafe(32);server.studio=Studio(args.storage,args.enable_github,args.test_mode)
