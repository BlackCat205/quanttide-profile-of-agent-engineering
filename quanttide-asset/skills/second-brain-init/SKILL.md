...
7. 查看 `result.md`、`execution-log.json` 和 `verification-report.json`。恢复时先按记录状态选择唯一入口：没有计划则重新规划，已确认且未完成则核对检查点后继续，操作全部完成则只重新验证。不要为某个 sample 名称增加例外，也不要通过改名重建来掩盖旧任务。

## 完成判断

程序会检查仓库与远端 HEAD、子模块 URL 与指针、目录、必要文档和 Markdown 基本格式。领域边界是否准确、人工资料是否适当、变更是否符合当前组织章程仍由人和 AI 审阅。报告分别写明程序验证、模拟反馈、真实用户验收及真实 GitHub 验证结果。远端读取失败表示“暂时无法核验”，不能当作产物不合格或名称可用；只读查询可有限重试，写入超时需先核对实际结果。计划内的子仓库已经下载并核验后，挂载必须复用该本地仓库，仅在 `.gitmodules` 保留公开 URL，不能为每个挂载重复下载。旧执行器的计划可以只读查看，但不能通过改写计划摘要或检查点来绕过版本校验。
