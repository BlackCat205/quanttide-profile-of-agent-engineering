...
程序会检查仓库与远端 HEAD、子模块 URL 与指针、目录、必要文档和 Markdown 基本格式。领域边界是否准确、人工资料是否适当、变更是否符合当前组织章程仍由人和 AI 审阅。报告分别写明程序验证、模拟反馈、真实用户验收及真实 GitHub 验证结果。远端读取失败表示“暂时无法核验”，不能当作产物不合格或名称可用；只读查询可有限重试，写入超时需先核对实际结果。旧执行器的计划可以只读查看，但不能通过改写计划摘要或检查点来绕过版本校验。
