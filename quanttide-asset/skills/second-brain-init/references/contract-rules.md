...
- Rust CLI 当前读取路径为 `.quanttide/asset/contract.yaml`。
- 插件交付仓库在该路径登记 `second-brain-init` 技能入口及交付资产，证明插件本体可发现、可调用。
- 插件生成或补全领域时，领域仓库在该路径登记六个实际挂载资产；总入口在同一路径登记领域挂载，证明生成结果可被资产云沿层级发现。
- 契约字段只使用当前 CLI 支持的结构；计划阶段展示完整 YAML，确认后才写入。
- 插件额外核对契约中的路径和仓库地址与 `.gitmodules` 一致。CLI 能解析 YAML 只证明格式可读，不能替代生成结果语义检查。
