...
`new_repositories_only: true` 仅用于新建领域，要求除总入口以外的目标仓库不存在，发现同名（包括空仓库）时停止，不自动复用。UI 0.4.0 默认启用。CLI 未设置时保留既有场景行为，维护者须审阅复用范围。GitHub 的不可见资源不能被断言为不存在；新建请求由 GitHub 原子地拒绝实际重名，创建失败后不会退回到复用已有仓库。

执行器 0.2.1 将仓库是否存在与是否有提交分别记录，并在每步执行前比较检查点。旧版本待执行计划应重新生成，不沿用旧确认；这不保证跨多个仓库的操作具有事务原子性。
